...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    setError('');
    try {
      const response = await http.post('/auth/forgot-password', { email, turnstile_token: turnstileToken });
  ...
  return (
    <div className="-mx-3 mt-6 max-w-md border border-gray-100 bg-white p-2 shadow-sm dark:border-slate-700 dark:bg-slate-800 min-[360px]:mx-auto min-[360px]:rounded-lg min-[360px]:p-4 sm:mt-10 sm:p-6">
      ...
      <form onSubmit={handleSubmit} className="mt-6 space-y-4">
        ...
        <TurnstileWidget action="forgot_password" onToken={setTurnstileToken} resetKey={resetKey} />
        <button type="submit" disabled={isSubmitting || (turnstileEnabled && !turnstileToken)} className="flex w-full items-center justify-center gap-2 rounded bg-primary p-2 text-white hover:bg-green-600 disabled:opacity-60">
