...
export default function StoryPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto bg-white dark:bg-slate-800 rounded-xl shadow-xs border border-gray-100 dark:border-slate-700 overflow-hidden mt-4 p-4 sm:p-6">
      <div className="md:flex gap-6">
        <div className="md:w-1/3 bg-gray-100 dark:bg-slate-700 rounded-lg overflow-hidden shrink-0">
          {story.coverImageUrl ? (
        ...
        <div className="mt-4 md:mt-0 md:w-2/3 flex flex-col justify-between">
          <div>
            ...
            <div className="flex items-center gap-1.5 text-base sm:text-lg text-gray-600 dark:text-gray-300 mb-3 sm:mb-4">
              <span>By</span>
              <Link to={`/profile/${story.authorName}`} className="font-medium hover:underline text-gray-900 dark:text-gray-100">
                {story.authorName}
              </Link>
            ...
            <div className="flex flex-wrap items-center gap-2 sm:gap-4 text-xs sm:text-sm text-gray-500 mb-6">
              <span className="px-2.5 py-1 bg-gray-100 dark:bg-slate-700 rounded-full">👁 {story.readCount} Reads</span>
              <button
                type="button"
                ...
                className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs sm:text-sm font-medium border transition-colors cursor-pointer ${
                  isLiked
                    ...
                    : 'bg-gray-50 border-gray-200 text-gray-600 hover:bg-gray-100 dark:bg-slate-700 dark:border-slate-600 dark:text-gray-300'
                }`}
              ...
              <span className="px-2.5 py-1 bg-gray-100 dark:bg-slate-700 rounded-full">{story.isCompleted ? 'Completed' : 'Ongoing'}</span>
            </div>
            ...
            <div className="flex flex-wrap gap-2 mb-8">
              {story.genres?.split(',').map((g: string) => (
                <span key={g} className="px-3 py-1 bg-gray-100 dark:bg-slate-700 rounded-full text-xs text-gray-600 dark:text-gray-300 font-medium">
                  {g.trim()}
                </span>
              ))}
            </div>
                ...
                {parts.map((part: any, index: number) => (
                  <li key={part.id}>
                    <Link 
                      ...
                      className="flex justify-between items-center gap-2 p-2.5 sm:p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700 transition border border-gray-100 dark:border-slate-700"
                    >
