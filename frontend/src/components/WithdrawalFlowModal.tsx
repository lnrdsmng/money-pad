...
export const WithdrawalFlowModal = ({ requestId, onClose }: { requestId: string; onClose: () => void }) => {
  ...
  if (isRequestError) {
    return (
      <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/50 p-4">
        <div role="alert" className="w-full max-w-sm rounded-xl bg-white dark:bg-slate-900 p-6 text-center shadow-xl">
          <p className="font-semibold text-red-700 dark:text-red-400">{getApiErrorMessage(requestError, 'The withdrawal details could not be loaded.')}</p>
          <div className="mt-5 flex justify-center gap-3">
            <button type="button" onClick={onClose} className="rounded-lg px-4 py-2 text-sm font-medium text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-slate-800">Close</button>
  ...
  if (!req) return <div className="fixed inset-0 bg-black/50 z-40 flex items-center justify-center"><div className="bg-white dark:bg-slate-900 text-gray-900 dark:text-slate-100 p-6 rounded text-center" role="status">Loading payout details...</div></div>;
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-40 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-slate-900 rounded-xl shadow-xl max-w-md w-full overflow-hidden relative">
        <button
          type="button"
          onClick={onClose}
        ...
        <div className="p-4 sm:p-6 text-center border-b border-gray-100 dark:border-slate-800">
        ...
        <div className="p-4 sm:p-6 bg-gray-50 dark:bg-slate-950 space-y-4 sm:space-y-6">
          {/* Fee & Net Breakdown */}
          <div className="bg-white dark:bg-slate-900 p-3 sm:p-4 rounded-lg border border-gray-200 dark:border-slate-800 text-xs sm:text-sm space-y-2">
            <div className="flex justify-between text-gray-600 dark:text-gray-400">
              <span>Gross Amount</span>
              <span className="font-semibold text-gray-900 dark:text-gray-100">₱{grossAmount.toFixed(2)}</span>
            </div>
            ...
            <div className="flex justify-between items-center text-gray-600 dark:text-gray-400">
              ...
              {isWaived ? (
                ...
                <span className="font-semibold text-red-500 dark:text-red-400">-₱{platformFee.toFixed(2)}</span>
              )}
            </div>
            ...
            {bankFee > 0 && (
              <div className="flex justify-between text-gray-600 dark:text-gray-400">
                ...
                <span className="font-semibold text-red-500 dark:text-red-400">-₱{bankFee.toFixed(2)}</span>
              </div>
            )}

            <div className="border-t border-gray-200 dark:border-slate-800 pt-2 mt-2 flex justify-between font-bold text-base text-gray-900 dark:text-gray-100">
              <span>Net Payout</span>
              <span className="text-emerald-600 dark:text-emerald-400 font-bold">₱{netAmount.toFixed(2)}</span>
            </div>
          </div>
          ...
          {isWaived ? (
            ...
            <div className="text-center">
              <p className="font-medium text-gray-900 dark:text-gray-100 text-sm mb-1.5">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mb-4">{req.ads_watched_count || 0} / 10 tasks completed</p>
              ...
              <div className="space-y-3">
                ...
                <button
                  ...
                  className="flex w-full items-center justify-center gap-2 rounded-lg bg-gray-200 dark:bg-slate-800 py-2.5 font-medium text-gray-700 dark:text-gray-200 text-sm hover:bg-gray-300 dark:hover:bg-slate-700 disabled:cursor-not-allowed disabled:opacity-60 transition-colors"
                >
