      ...
      <div className="relative max-h-[92vh] w-full max-w-6xl overflow-y-auto rounded-2xl bg-white shadow-xl">
        ...
        <div className="space-y-6 bg-slate-50 p-5 sm:p-8">
          ...
          {!pendingPurchase && latestRejected?.rejection_reason && (
            <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-red-800">Latest payment was rejected: {latestRejected.rejection_reason}</div>
          )}
          <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-4">
            {(plansQuery.data ?? []).map((plan) => {
              ...
              return (
                <article key={plan.id} className={`rounded-xl border-2 bg-white p-6 shadow-sm ${isCurrent ? 'border-emerald-500' : selectedPlan?.id === plan.id ? 'border-blue-500' : 'border-transparent'}`}>
                  <Icon className="h-8 w-8 text-slate-700" />
                  <h3 className="mt-4 text-xl font-bold">{plan.name}</h3>
                  <p className="mt-2 text-3xl font-extrabold">₱{Number(plan.price).toFixed(0)}</p>
