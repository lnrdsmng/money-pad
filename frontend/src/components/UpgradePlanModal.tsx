...
const planStyles: Record<ReadingPlanId, { icon: typeof Shield; buttonClass: string }> = {
  free: { icon: Shield, buttonClass: 'bg-slate-700 hover:bg-slate-800 text-white' },
  standard: { icon: Sparkles, buttonClass: 'bg-primary hover:bg-primary-hover text-white' },
  mega_premium: { icon: Zap, buttonClass: 'bg-primary hover:bg-primary-hover text-white' },
  ultimate_premium: { icon: Crown, buttonClass: 'bg-accent hover:bg-accent-hover text-white' },
};
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" role="dialog" aria-modal="true">
      <div className="relative max-h-[92vh] w-full max-w-6xl overflow-y-auto rounded-2xl bg-white dark:bg-slate-900 shadow-xl">
        ...
        <div className="space-y-6 bg-slate-50 dark:bg-slate-950 p-3.5 sm:p-8">
          {pendingPurchase && (
            <div className="rounded-xl border border-amber-200 dark:border-amber-900/50 bg-amber-50 dark:bg-amber-950/40 p-3 sm:p-4 text-xs sm:text-sm text-amber-900 dark:text-amber-200">
          ...
          {!pendingPurchase && latestRejected?.rejection_reason && (
            <div className="rounded-xl border border-red-200 dark:border-red-900/50 bg-red-50 dark:bg-red-950/40 p-3 sm:p-4 text-xs sm:text-sm text-red-800 dark:text-red-200">Latest payment was rejected: {latestRejected.rejection_reason}</div>
          )}
          ...
          {selectedPlan && !pendingPurchase && (
            <form onSubmit={submit} className="mx-auto max-w-2xl space-y-4 sm:space-y-5 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 sm:p-6 text-slate-900 dark:text-slate-100">
              ...
              <div><label className="mb-1 block text-sm font-semibold text-slate-800 dark:text-slate-200">Payment screenshot</label><label className={`flex items-center gap-3 rounded-lg border border-dashed border-slate-400 dark:border-slate-600 bg-white dark:bg-slate-800/50 text-slate-700 dark:text-slate-300 p-4 ${submitMutation.isPending ? 'cursor-not-allowed opacity-60' : 'cursor-pointer'}`}><Upload className="h-5 w-5" /><span>{paymentProof?.name ?? 'Choose JPEG, PNG, or WebP (max 5 MB)'}</span><input required disabled={submitMutation.isPending} type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => setPaymentProof(event.target.files?.[0] ?? null)} className="sr-only" /></label></div>
              {submitMutation.isError && <p className="text-sm text-red-600 dark:text-red-400">The proof could not be submitted. Check the file and account name, then try again.</p>}
